...
async function generarPdf(payload) {
  ...
  if (logo) {
    doc.image(logo, 30, 30, { height: 48 });
  }
  if (personaje) {
    const anchoP = 90;
    doc.image(personaje, anchoPagina - 30 - anchoP, 30, { width: anchoP });
  }

  doc.font('Helvetica-Bold').fontSize(16).fillColor(colorPrimario).text('ACREDITACIÓN AL ENCUENTRO', 30, 105, {
    align: 'center',
    width: anchoPagina - 60,
  });
  doc.font('Helvetica').fontSize(11).fillColor('#334155').text(`Código único: ${datos.id}`, 30, 128, {
    align: 'center',
    width: anchoPagina - 60,
  });
  ...
  doc.text(`${datos.apellido || ''} ${datos.nombre || ''}`.trim(), inicioX, y);
  y += 18;
  ...
  doc.font('Helvetica-Bold').fontSize(11).fillColor(colorPrimario).text('Talleres:', inicioX, y);
  y += 16;
  ...
  for (const s of sesiones) {
    const turno = s.turno === 'manana' ? 'Mañana' : s.turno === 'tarde' ? 'Tarde' : s.turno;
    const partes = [`${s.taller || 'Taller'} (${turno})`];
    if (s.fecha) partes.push(`Fecha: ${s.fecha}`);
